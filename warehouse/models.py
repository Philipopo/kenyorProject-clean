...
class WarehouseItem(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=['item', 'storage_bin'], name='unique_warehouse_item_bin')
        ]
    ...
    def clean(self):
        # Validate storage_bin capacity
        if self.storage_bin:
            total_used = self.storage_bin.warehouse_items.exclude(id=self.id).aggregate(
    ...
    def save(self, *args, **kwargs):
        ...
        if self.storage_bin:
            self.storage_bin.update_used()  # Update StorageBin.used